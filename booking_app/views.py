...
def admin_email_template_form_view(request, pk=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            #send_system_notification_task.delay(event, template=instance, user=request.user)
...
def admin_email_template_test_view(request, pk):
    ...
    #    'temp_password': 'test_password_123',
    ...
    try:
        send_system_notification_task.delay(
            ...
            context_data={
                "booking_id": mock_booking.pk,
                "user_id": test_user.pk,
                "vehicle_id": test_vehicle.pk,
                "location_id": test_location.pk,
                "temp_password": "test_password_123",
                "domain": request.get_host(),
            },
    ...
    if request.method == 'POST':
        ...
        if action == 'approve':
            if booking.status == 'pending':
                ...
                send_system_notification_task.delay('booking_approved', context_data=safe_context(ctx))
            ...
            if booking.vehicle.vehicle_type == 'APV' and booking.status == 'pending':
                ...
                send_system_notification_task.delay('apv_booking_approved', context_data=safe_context(ctx))
            ...
            if booking.status in ['pending', 'pending_contract', 'pending_final_km', 'confirmed']:
                ...
                send_system_notification_task.delay('booking_canceled_by_manager', context_data=safe_context(ctx))
            ...
            if booking.status == 'confirmed':
                ...
                booking.save(update_fields=update_fields)
                send_system_notification_task.delay('booking_ended_pending_km', context_data=safe_context(ctx))
