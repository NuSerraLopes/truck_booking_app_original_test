...
# --- HELPER FUNCTION FOR BOOKING SUBMISSION ---
        ...
        if resolution:
            ...
            if not existing_clients.exists():
                ...
                for existing_client in existing_clients:
                    changes = {}
                    if form_data['name'] and form_data['name'] != existing_client.name: changes['name'] = {
                        'old': existing_client.name, 'new': form_data['name']}
                    if form_data['address'] and form_data['address'] != existing_client.address: changes['address'] = {
                        'old': existing_client.address, 'new': form_data['address']}
                    if form_data['email'] and form_data['email'] != existing_client.email: changes['email'] = {
                        'old': existing_client.email, 'new': form_data['email']}
                    if form_data['phone_number'] and form_data['phone_number'] != existing_client.phone_number: changes[
                        'phone_number'] = {'old': existing_client.phone_number, 'new': form_data['phone_number']}
                ...
                if client_to_silently_update:
                    update_fields = {k: v for k, v in form_data.items() if
                                     v and not getattr(client_to_silently_update, k)}
                    if update_fields:
...
def vehicle_list_view(request, group_name=None):
    ...
    # --- Base queryset with performance optimizations (remains the same) ---
    ...
    # Priority 1: Admins and Booking Admins see everything by default
    if not group_name and request.user.is_booking_admin_member:
        pass  # Do nothing, show all vehicles
    else:
        # Priority 2: An explicit filter in the URL (e.g., /vehicles/heavy/)
        effective_group_for_filter = None
        if group_name:
            effective_group_for_filter = group_name.lower()
        # Priority 3: An automatic filter based on the non-admin user's group
        else:
            user_groups = request.user.groups.all()
            for group in user_groups:
                normalized_user_group_name = group.name.lower()
                if normalized_user_group_name in ['light', 'tllight', 'heavy', 'tlheavy', 'apv', 'tlapv']:
    ...
    # --- Sorting application (remains the same) ---
    ...
    for vehicle in page_obj:
        ...
        if vehicle.availability_slots and vehicle.availability_slots[0]['start'] <= (today + timedelta(days=1)):
...
def vehicle_detail_view(request, pk):
    ...
    context = {'vehicle': vehicle, 'availability_slots': availability_slots, 'tomorrow': tomorrow,
...
def book_vehicle_view(request, vehicle_pk):
    ...
    unavailable_ranges = [{"start": subtract_business_days(b.start_date, 3).strftime('%Y-%m-%d'),
                           "end": add_business_days(b.end_date, 3).strftime('%Y-%m-%d')} for b in all_bookings]
    context = {'form': form, 'vehicle': vehicle, 'unavailable_ranges_json': json.dumps(unavailable_ranges),
...
def group_dashboard_view(request):
    ...
    base_query = Booking.objects.filter(vehicle__vehicle_type__in=vehicle_types_to_manage).select_related('client',
                                                                                                          'vehicle')
    if selected_status:
        actionable_bookings = base_query.filter(status=selected_status).order_by('-start_date')
    else:
        actionable_bookings = base_query.filter(status__in=['pending', 'pending_contract', 'confirmed'],
                                                end_date__gte=timezone.now().date()).order_by('start_date')
    context = {'page_title': _("Group Dashboard"), 'actionable_bookings': actionable_bookings,
...
def group_reports_view(request):
    ...
    bookings_per_month = Booking.objects.filter(vehicle__vehicle_type__in=vehicle_types_to_manage,
                                                start_date__gte=twelve_months_ago,
                                                status__in=['confirmed', 'completed', 'on_going',
                                                            'pending_final_km']).annotate(
        month=TruncMonth('start_date')).values('month').annotate(count=Count('id')).order_by('month')
    ...
    vehicle_usage = Booking.objects.filter(vehicle__vehicle_type__in=vehicle_types_to_manage,
                                           status__in=['confirmed', 'completed', 'on_going',
                                                       'pending_final_km']).values('vehicle__license_plate').annotate(
    ...
    context = {'page_title': _("Group Reports & Charts"), 'bookings_chart_labels': json.dumps(bookings_chart_labels),
               'bookings_chart_data': json.dumps(bookings_chart_data),
               'vehicle_chart_labels': json.dumps(vehicle_chart_labels),
               'vehicle_chart_data': json.dumps(vehicle_chart_data), 'vehicle_usage_table': vehicle_usage, }
...
def group_calendar_view(request):
    vehicle_types_to_manage = get_managed_vehicle_types(request.user)
    calendar_bookings = Booking.objects.filter(vehicle__vehicle_type__in=vehicle_types_to_manage,
                                               status__in=['pending', 'pending_contract', 'confirmed', 'on_going',
                                                           'pending_final_km']).select_related('vehicle', 'client')
    ...
    colors = ['#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#bcf60c',
              ...
              '#000075', '#808080']
    license_plate_color_map = {vehicle.license_plate: colors[i % len(colors)] for i, vehicle in
                               enumerate(sorted(unique_vehicles, key=lambda v: v.license_plate))}
    calendar_events = []
    for booking in calendar_bookings:
        client_name = booking.client.name if booking.client else "N/A"
        calendar_events.append({'id': booking.pk, 'text': f"{booking.vehicle.license_plate} - {client_name}",
    ...
    context = {'page_title': _("Group Bookings Calendar"), 'calendar_events': json.dumps(calendar_events),
...
def admin_dashboard_view(request):
    ...
    context = {'page_title': _("Admin Dashboard"), 'recent_email_logs': recent_email_logs}
...
def vehicle_delete_view(request, pk):
    ...
    if vehicle_to_delete.bookings.exists():
        messages.error(request,
                       _(f"Vehicle '{vehicle_to_delete.license_plate}' cannot be deleted because it has associated bookings."))
        return redirect(reverse('booking_app:admin_vehicle_list'))
    if request.method == 'POST':
        vehicle_to_delete.delete()
        messages.success(request, _(f"Vehicle '{vehicle_to_delete.license_plate}' deleted successfully!"))
        return redirect(reverse('booking_app:admin_vehicle_list'))
    context = {'vehicle_obj': vehicle_to_delete}
...
def admin_vehicle_list_view(request):
    ...
    active_booking = Booking.objects.filter(vehicle=OuterRef('pk'), status__in=['confirmed', 'on_going'],
                                            start_date__lte=today, end_date__gte=today)
    vehicles_qs = Vehicle.objects.select_related('current_location').annotate(
        annotated_is_available=Case(When(Exists(active_booking), then=Value(False)), default=Value(True),
                                    output_field=BooleanField()),
        current_customer=Subquery(active_booking.values('client__name')[:1]))
    if filter_value:
        if filter_by == 'license_plate':
            ...
            vehicles_qs = vehicles_qs.filter(
                Q(license_plate__icontains=filter_value) | Q(model__icontains=filter_value) | Q(
                    current_customer__icontains=filter_value) | Q(current_location__name__icontains=filter_value))
    order_by_field = 'current_location__name' if sort_by == 'current_location' else sort_by
    if direction == 'desc':
        order_by_field = f'-{order_by_field}'
    vehicles_qs = vehicles_qs.order_by(order_by_field)
    ...
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    context = {'vehicles': page_obj, 'page_title': _("Manage Vehicles"), 'current_sort': sort_by,
               'current_dir': direction, 'opposite_dir': 'desc' if direction == 'asc' else 'asc',
               'vehicle_type_choices': Vehicle.VEHICLE_TYPE_CHOICES, 'all_locations': Location.objects.all(), }
...
def admin_user_reset_password_view(request, pk):
    ...
    return render(request, 'admin/admin_user_reset_password.html', {'form': form, 'user_to_reset': user_to_reset})
...
def send_credentials_view(request, pk):
    ...
    send_booking_notification(event_trigger='send_user_credentials',
...
def send_temporary_password_view(request, pk):
    ...
    user_to_reset.requires_password_change = True
    user_to_reset.save()
    send_booking_notification(event_trigger='send_temporary_password',
...
def location_edit_view(request, pk):
    ...
    return render(request, 'admin/admin_location_edit.html', {'form': form, 'location_obj': location_to_edit})
...
def location_delete_view(request, pk):
    ...
    if Booking.objects.filter(Q(start_location=location_to_delete) | Q(end_location=location_to_delete)).exists():
        messages.error(request,
                       _(f"Location '{location_to_delete.name}' cannot be deleted because it is used in existing bookings."))
        return redirect(reverse('booking_app:admin_location_list'))
    if request.method == 'POST':
        location_to_delete.delete()
        messages.success(request, _(f"Location '{location_to_delete.name}' deleted successfully!"))
        return redirect(reverse('booking_app:admin_location_list'))
    return render(request, 'admin/admin_location_confirm_delete.html', {'location': location_to_delete})
...
def group_edit_view(request, pk):
    ...
    return render(request, 'admin/admin_group_form.html', {'form': form, 'group_obj': group_to_edit})
...
def group_delete_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect(reverse('booking_app:admin_group_list'))
    return render(request, 'admin/admin_group_delete.html', {'group_obj': group_to_delete})
...
def admin_email_template_form_view(request, pk=None):
    ...
    return render(request, 'admin/admin_email_template_form.html', {'form': form})
...
def admin_email_template_test_view(request, pk):
    ...
    if not test_vehicle or not test_location:
        messages.error(request, _("Cannot run test: A vehicle and location must exist."))
        return redirect('booking_app:admin_email_template_list')
    ...
    try:
        send_booking_notification(event_trigger=template.event_trigger, booking_instance=mock_booking,
                                  context_data=context_data, test_email_recipient=request.user.email)
...
def email_log_list_view(request):
    ...
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    return render(request, 'admin/admin_email_log_list.html', {'page_obj': page_obj})
...
def admin_dl_form_view(request, pk=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, _(f"Distribution list saved successfully!"))
            return redirect('booking_app:admin_dl_list')
    else:
        form = DistributionListForm(instance=instance)
    return render(request, 'admin/admin_dl_form.html', {'form': form})
...
def admin_dl_delete_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('booking_app:admin_dl_list')
    return render(request, 'admin/admin_dl_confirm_delete.html', {'distribution_list': distribution_list})
...
def check_client_in_db_view(request):
    ...
    if clients.exists():
        client_data = [
            {'id': c.id, 'name': c.name, 'address': c.address, 'email': c.email, 'phone_number': c.phone_number} for c
            in clients]
...
def get_company_details_view(request):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "Não existe qualquer certidão com esse número." in response.text:
            ...
            if not details_table: return JsonResponse({'error': 'Details table not found'}, status=500)
            details_td = details_table.find_all('tr')[1].find('td')
            if not details_td: return JsonResponse({'error': 'Details cell not found'}, status=500)
        ...
        return JsonResponse({'error': f'Server responded with status {response.status_code}'}, status=500)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('isValid', False):
                return JsonResponse(
                    {'valid': True, 'company_name': data.get('name', ''), 'address': data.get('address', '')})
            else:
...
def my_bookings_api_view(request):
    ...
    for booking in user_bookings:
        ...
        events.append({'id': booking.pk, 'text': f"{booking.vehicle.license_plate} - {client_name}",
                       'start': booking.start_date.isoformat(),
                       'end': (booking.end_date + timedelta(days=1)).isoformat(),
                       'url': reverse('booking_app:booking_detail', kwargs={'booking_pk': booking.pk}),
                       'backColor': {'LIGHT': '#3c78d8', 'HEAVY': '#cc0000', 'APV': '#6aa84f'}.get(
                           booking.vehicle.vehicle_type, '#dddddd'), })
...
def booking_api_view(request):
    ...
    for booking in all_bookings:
        client_name = booking.client.name if booking.client else _("N/A")
        event_list.append({"id": booking.pk, "text": f"{booking.vehicle.license_plate} - {client_name}",
                           "start": booking.start_date.isoformat(),
                           "end": (booking.end_date + timedelta(days=1)).isoformat(), })
